...
try{
    ...
    Write-au2matorLog -Type INFO -Text "Try to get the Team"
    
    $Team=Get-Team -GroupID $C_TeamID

    if ($Team)
    {
        Write-au2matorLog -Type INFO -Text "Team found: $($Team.Displayname)"
        ...
        try{
            ...
            Write-au2matorLog -Type ERROR -Text "Could not remove Team"
            $ErrorCount = 1
        }
    }
    else {
        Write-au2matorLog -Type ERROR -Text "Team not found"
        $ErrorCount = 1
    }
...
if ($ErrorCount -eq 0) {
    $au2matorReturn = "Team has been removed"
    $AdditionalHTML = "
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to remove Teams, Error: $Error"
